...
def get_latest_workout(exercise_id):
	with get_connection() as conn:
		with conn.cursor(cursor_factory=RealDictCursor) as cur:
			cur.execute("""
...
def get_workout_history(limit=10):
	with get_connection() as conn:
		with conn.cursor(cursor_factory=RealDictCursor) as cur:
			cur.execute("""
				SELECT w.workout_date, e.name, w.sets, w.reps, w.weight_kg, w.volume_kg
				FROM gym.workouts w
				JOIN gym.exercises e ON w.exercise_id = e.id
				ORDER BY w.workout_date DESC, w.created_at DESC
				LIMIT %s
			""", (limit,))
			return cur.fetchall()

def get_previous_workout(exercise_id, exclude_id=None):
	with get_connection() as conn:
		with conn.cursor(cursor_factory=RealDictCursor) as cur:
			cur.execute("""
				SELECT sets, reps, weight_kg, volume_kg, workout_date
